...
to_char(sysdate, 'fmdd-mm-yy'), -- fm은 앞에숫자를 없애준다. 07 -> 7
to_char(sysdate, 'yyy-mm-dd'),
to_char(sysdate, 'YYYY-MM-DD HH:MI:SS.SSSSS'), -- 04시
to_char(sysdate, 'YYYY-MM-DD HH24:MI:SS.SSSSS AM'), -- 16시
to_char(sysdate, 'YYYY-MON-DD HH24:MI:SS.SSSSS AM')
from dual;
...
--1,020,222 -- 숫자를 화폐단위로 바꿀 때 : 콤마 들어가면 문자 10,000
...
-- decode -- DECODE 함수는 Oracle 구문에만 해당됩니다.
select first_name, last_name, salary, 
...
from hr.employees
group by department_id, job_id -- select절에 있는 컬럼명은 반드시 group by절에 써야한다. group by절에 있는 컬럼명은 select절에 안써도 가능. 
